function results = predvariance21_coeff(usedata,usemats,numfac)
...
for b=1:B
    ...
    pcoeff_boot(:,:,b)=pcoeff_b;
    
    YY=Y(usam,:);
    ybar_sam=YY(:,shortix);
    XX=[ones(size(ybar_sam,1),1) ybar_sam];
    coeff_sam=(XX'*XX)\(XX'*YY);
    ac_b=coeff_sam(2:end,:)';
    acoeff_boot(:,:,b)=ac_b;
...
for j=1:numfac
    ...
    results.pcoeff_perchigh(:,j)=prctile(squeeze(pcoeff_boot(:,j,:))',97.5)';
    results.pcoeff_perclow(:,j)=prctile(squeeze(pcoeff_boot(:,j,:))',2.5)';

    results.acoeff_perchigh(:,j)=prctile(squeeze(acoeff_boot(:,j,:))',97.5)';
    results.acoeff_perclow(:,j)=prctile(squeeze(acoeff_boot(:,j,:))',2.5)';
